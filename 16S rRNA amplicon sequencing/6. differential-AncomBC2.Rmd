...
# Run ANCOMBC2 - Global analysis
# Jalankan ANCOMBC2 untuk analisis global berdasarkan "Location"
output1 <- ancombc2(
  data = pseq,
  tax_level = "Genus",
...
```{r}
# ===================================
# 1. PAIRWISE test (Location sample set)
# ===================================
library(phyloseq)
library(ANCOMBC)
library(dplyr)
...
# 1) Tentukan nama kolom yang ada (otomatis)
lfc_cols <- grep("^lfc_", names(res_pair), value = TRUE)
q_cols   <- grep("^q_", names(res_pair), value = TRUE)

# Print kolom lfc/q untuk verifikasi
message("LF C columns found:\n", paste(lfc_cols, collapse = "\n"))
message("Q value columns found:\n", paste(q_cols, collapse = "\n"))

# 2) Pemetaan perbandingan yang kita butuhkan (sesuai struktur model)
# Interpretasi berdasarkan struktur yang kamu laporkan:
...
# 4) Buat barplot terpisah (satu plot per perbandingan)
...
# 5) Volcano plot terpisah (tandai signifikan berdasarkan kolom diff_... jika ada, otherwise berdasarkan q)
# Untuk NangRong vs LahanSai
p_volcano1 <- res_pair %>%
