...
Reference:
The code and workflow used in this study were adapted from Lin (2024) with minor modifications, as cited in the references and described in the ANCOMBC Bioconductor package documentation (https://www.bioconductor.org/packages/release/bioc/html/ANCOMBC
...
# res_global = ANCOM-BC2 global test
res_plot <- res_global %>%
  dplyr::mutate(
    neg_log_q = -log10(q_val),
    signif = ifelse(diff_robust_abn, "Significant & Robust", "Not Significant")
...
ggplot(sig_res, aes(x = reorder(taxon, lfc), y = lfc, fill = comparison)) +
  ...
  coord_flip() +
  theme_minimal() +
  labs(title = "Significant Genera (|lfc| ≥ 2, q<0.05)",
...
barplot1 = res_pair %>%
  ...
  coord_flip() +
  theme_minimal() +
  labs(title = "NangRong vs LamLukKa", x = "Genus", y = "Log2 Fold Change")
...
p_volcano1 <- res_pair %>%
  ...
  theme_minimal() +
  labs(title = "Volcano: NangRong vs LahanSai", x = "Log2 Fold Change", y = "-log10(q-value)")
print(p_volcano1)
...
p_volcano2 <- res_pair %>%
  ...
  scale_color_manual(values = c("FALSE" = "grey50", "TRUE" = "red")) +
  theme_minimal() +
  labs(title = "Volcano: NangRong vs LamLukKa", x = "Log2 Fold Change", y = "-log10(q-value)")
print(p_volcano2)
-------------------------------------------------------------------------------------------------------------------
...
if (nrow(res_nang_vs_lahan) > 0) {
  p_nang_vs_lahan <- ggplot(res_nang_vs_lahan,
    ...
    labs(title = "NangRong vs LahanSai", x = "Genus", y = "Log2 Fold Change") +
     theme(
...
if (nrow(res_lam_vs_lahan) > 0) {
  p_lam_vs_lahan <- ggplot(res_lam_vs_lahan,
    ...
    labs(title = "LamLukKa vs LahanSai", x = "Genus", y = "Log2 Fold Change") +
     theme(
  axis.title.x = element_text(face = "bold", size = 12),  # Log2 Fold Change
