...
p4a1 <- ggplot(Taxonomies_x, aes(x = group, y = Abundance, fill = aa, order = aa)) +
  ...
  theme_classic() +
  theme(
    panel.background = element_rect(fill = "transparent", color = NA),  # 📌 Hapus background panel
    plot.background = element_rect(fill = "transparent", color = NA),   # 📌 Hapus background plot
